...
```{r echo=FALSE}
data.table(Class = names(table(train$target)),
           "Training Set" = c(table(train$target)))[,
              "Percentage (Training)" := round(`Training Set`/sum(`Training Set`)*100,
                                   digits = 3)
           ][, "Test Set" := c(table(test$target))
...
The bottom 13 principal components explain only `r round(sum(pca.var[21:33]), digits=2)` % of total variance. These can be dropped (dimensional reduction).
...
Drop the last 6 principal components since they together explain only `r sum(pca_dummy.var[15:20])` % of total variance.
...
- KDD CUP 99: [http://kdd.ics.uci.edu/databases/kddcup99/kddcup99.html](http://kdd.ics.uci.edu/databases/kddcup99/kddcup99.html)
